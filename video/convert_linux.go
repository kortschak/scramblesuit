...
// uyvyToImage converts a UYVY 4:2:2 frame to an NRGBA image.
//
// UYVY packs two pixels into every four bytes:
//
//	[U Y0 V Y1]
//
// Each pixel pair shares U (Cb) and V (Cr) chroma samples.
func uyvyToImage(data []byte, width, height int) image.Image {
	img := image.NewNRGBA(image.Rect(0, 0, width, height))
	for y := 0; y < height; y++ {
		off := y * width * 2
		for x := 0; x < width; x += 2 {
...
// imageToUYVY converts an image to UYVY 4:2:2 packed bytes.
//
// Each pair of horizontal pixels shares chroma (Cb/Cr) samples,
// averaged from both pixels:
//
//	[U Y0 V Y1]
func imageToUYVY(img image.Image) []byte {
	bounds := img.Bounds()
	width := bounds.Dx()
	height := bounds.Dy()
	data := make([]byte, width*height*2)
	for row := 0; row < height; row++ {
		...
		for col := 0; col < width; col += 2 {
			...
			i := off + col*2
			data[i] = uint8((uint16(cb0) + uint16(cb1)) / 2)
			data[i+1] = y0
			data[i+2] = uint8((uint16(cr0) + uint16(cr1)) / 2)
			data[i+3] = y1
		}
	}
	return data
